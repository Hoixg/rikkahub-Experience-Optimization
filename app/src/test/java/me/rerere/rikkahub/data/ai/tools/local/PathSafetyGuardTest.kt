...
class PathSafetyGuardTest {
    ...
    @Test fun `own work profile data path is allowed`() {
        assertNull(PathSafetyGuard.check("/data/user/10/${BuildConfig.APPLICATION_ID}/files/prefs.json"))
    }

    @Test fun `own device protected data path is allowed`() {
        assertNull(PathSafetyGuard.check("/data/user_de/0/${BuildConfig.APPLICATION_ID}/files/prefs.json"))
    }
    ...
    @Test fun `other app user alias is blocked`() {
        val alias = PathSafetyGuard.check("/data/user/10/com.other.app/databases/secret.db")
        assertNotNull(alias)
        assertEquals("path_blocked", alias!!.code)
    }
